...
import { View, Animated, Dimensions, ListViewDataSource, Platform } from "react-native";
...
export default class ParallaxScroll extends Component {
  static propTypes = {
    ...
    isParallaxAdsorb: PropTypes.bool,
  ...
  _onScrollEndDrag(e) {
    ...
    let velocityY = e.nativeEvent.velocity.y;
    velocityY = Platform.OS === 'android' ? velocityY : -velocityY;
    const {
      isParallaxAdsorb,
      parallaxHeight
    } = this.props;
    if (isParallaxAdsorb) {
      if (contentOffsetY - velocityY * 40 < parallaxHeight / 2) {
        this.scrollRef.scrollTo({ 'y': 0, 'animated': true })
      } else {
        this.scrollRef.scrollTo({ 'y': parallaxHeight, 'animated': true })
      }
    }
  }
