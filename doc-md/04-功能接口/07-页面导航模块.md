...
* [miot/host/ui](#module_miot/host/ui)
    * [~IUi](#module_miot/host/ui..IUi)
        ...
        * [.goCustomPlugin()](#module_miot/host/ui..IUi+goCustomPlugin)
...
| option.force | <code>boolean</code> |  | 强制弹出隐私弹框，默认为false。对于共享设备，不建议进行弹窗请求隐私授权，如果一定要弹框，需要设置option.force=true， option.force=false会直接返回失败(相当于拒绝授权)。对于非共享设备，option.force可以不传 |
...
<a name="module_miot/host/ui..IUi+goCustomPlugin"></a>
...
**Example**  
```js
Host.ui.goCustomPlugin({"std_dimiss":true});
```
