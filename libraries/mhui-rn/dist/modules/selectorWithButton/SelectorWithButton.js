...
export default class SelectorWithButton extends Component {
  ...
  select = index => {
    ...
    if (hasChanged) {
      onSelected(selectedIndexs); // return {
  ...
  render() {
    ...
    return <Wrap>
        <View style={Styles.container}>
          ...
          {showSwitch && !switchOn ? null : <View style={[Styles.selectors, title || subtitle || showSwitch ? Styles.selectorsWithHeader : null, items.length >= 4 ? null : [Styles.selectorsPadding0, Styles.selectorsPadding1, Styles.selectorsPadding2, Styles.selectorsPadding3][sizeLevel]]}>
              {this.getSelectors()}
            </View>}
        </View>
      </Wrap>;
