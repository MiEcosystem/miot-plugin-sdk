...
export default class CardBase extends React.Component {
  ...
  renderInner() {
    if (this.props.innerView === undefined) {
      ...
      return <Animated.View style={[styles.innerContainer, {
        ...
        name: 'activate'
          ...
          {icon ? <Image style={[styles.innerIcon, iconStyle]} source={icon} resizeMode="contain" {...getAccessibilityConfig({
          accessible: false
        })} /> : null}
          <Text style={[styles.innerText, {
          color: theme.colorBlack
        }, textStyle]} numberOfLines={textLine} ellipsizeMode="tail" allowFontScaling={this.props.allowFontScaling} {...getAccessibilityConfig({
          accessible: true
        })}>
            {text || ''}
          </Text>
  ...
  renderClose() {
    ...
    return <TouchableHighlight style={styles.closeArea} underlayColor="transparent" onPress={() => this.dismiss()} {...getAccessibilityConfig({
      accessible: this.props.accessible,
      ...
      accessibilityHint: this.props.dismissAccessibilityHint || 'close'
        ...
        <Animated.Image style={[styles.close, {
        opacity: this.opacity
  ...
  render() {
    ...
    if (!this.props.visible) {
      // 消失
      Animated.parallel([Animated.timing(this.height, {
        toValue,
      ...
      // 出现
      Animated.parallel([Animated.timing(this.opacity, {
        toValue,
    ...
    const {
      animatedViewStyle,
      containerStyle
    } = this.getCorrectStyle(cardStyle);
    return <Animated.View ref={ref => {
      ...
      height: this.props.unlimitedHeightEnable ? undefined : this.height.interpolate({
        ...
        flex: 1
      }]} underlayColor={this.props.underlayColor} disabled={this.props.disabled} onPress={this.props.onPress} {...getAccessibilityConfig({
        accessible: false
      })}>
          <View style={{
          flex: 1
...
const styles = StyleSheet.create({
  container: {
    ...
    width: DEFAULT_STYLE.WIDTH // borderRadius: DEFAULT_STYLE.RADIUS
