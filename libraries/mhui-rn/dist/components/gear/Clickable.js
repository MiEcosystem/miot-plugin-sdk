...
export default class Clickable extends React.Component {
  ...
  render() {
    ...
    // 文本颜色不能应用 animation
    const color = this.props.select ? '#fff' : TEXT_COLOR; // 容器 border 变化

    const boderStyle = this.props.select ? {
      borderWidth: 0
    } : {
    ...
    const backgroundColor = this.props.select ? this.props.selectColor : '#fff'; // const backgroundColor = this.animatedValue.interpolate({
    //   inputRange: [0, 1],
    ...
    return <Animated.View style={[styles.animationContainer, boderStyle, this.props.style, {
      backgroundColor
    }]}>
        <TouchableWithoutFeedback onPress={this.props.onPress} {...getAccessibilityConfig({
