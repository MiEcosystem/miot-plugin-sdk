...
class InputDialog extends React.Component {
  ...
  constructor(props, context) {
    ...
    RkTheme.setType('RkTextInput', 'mhtextinput', {
      ...
      underlineWidth: 0.3,
      marginVertical: 0,
      placeholderTextColor: this.context.theme?.colorGrayLighter,
      minHeight: 40,
      backgroundColor: this.context.theme?.colorBackground,
      borderRadius: 6,
      borderWidth: 0.3,
      borderColor: this.context.theme?.colorSeparator
    });
    this.process(props);
  }
  ...
  process(props) {
    ...
    this.hasPressUnderlineText = false; // 拦截确认按钮的回调函数，传入 InputDialog 的一些信息
  ...
  renderInputGroup() {
    return this.inputs.map((input, index) => {
      ...
      return <RkTextInput key={index + (input.placeholder || '')} style={margin} placeholder={input.placeholder || ''} onChangeText={input.onChangeText} rkType="mhtextinput" defaultValue={input.defaultValue || ''} placeholderTextColor="#fff" {...input.textInputProps || {}} {...getAccessibilityConfig({
        accessible: this.props.accessible,
  ...
  render() {
    ...
    return <AbstractDialog animationType={this.props.animationType} visible={this.props.visible} title={this.props.title} buttons={this.buttons} dialogStyle={this.props.dialogStyle} onDismiss={() => this._onDismiss()} style={absDialogStyle} {...getAccessibilityConfig({
      accessible: this.props.accessible
    })}>
