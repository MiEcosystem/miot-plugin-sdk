...
export default class CryptoDemo extends React.Component {
  ...
  constructor(props, context) {
    ...
    this.cryptoObject = new ECCCrypto(ECCCrypto.CurveTypeSecp128r1);
    this.cryptoObject2 = new ECCCrypto(ECCCrypto.CurveTypeSecp128r1);
  }
          ...
          <TouchableOpacity
            onPress={() => {
              Promise.all([this.cryptoObject.generateKeyPair(), this.cryptoObject2.generateKeyPair()]).then((publicKeys) => {
                ...
                this.cryptoECCPublicKey = publicKeys[1];
              }).catch((err) => {
                console.log(`genrate key pair error:${ err }`);
              });
            }}
          >
            <Text style={[{ fontSize: 20 }, fontFamily]}>generate key pair</Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={() => {
              this.cryptoObject.generateSharedSecret(this.cryptoECCPublicKey).then((sharedSecret) => {
                this.cryptoECCSharedSecret = sharedSecret;
                console.log(`shared secret:${ sharedSecret }`);
              }).catch((err) => {
                console.log(`generate shared secret error:${ err }`);
              });
            }}
          >
            <Text style={[{ fontSize: 20 }, fontFamily]}>generate shared secret</Text>
          </TouchableOpacity>
