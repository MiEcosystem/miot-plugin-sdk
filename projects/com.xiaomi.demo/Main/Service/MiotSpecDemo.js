            ...
            <View style={{ flex: 1 }}>
                <ScrollView style={styles.containerMenu}>
                    <View style={{ height: 50, flexDirection: 'row', marginBottom: 8 }}>
                        <TouchableOpacity onPress={() => { this.setState({ selectBtn: 1 }) }} style={[styles.topButton, this.state.selectBtn == 1 && styles.topButtonSelected]}><Text style={[styles.topButtonText, this.state.selectBtn == 1 && styles.topButtonTextSelected]}>设置属性</Text></TouchableOpacity>
                        <TouchableOpacity onPress={() => { this.setState({ selectBtn: 2 }) }} style={[styles.topButton, this.state.selectBtn == 2 && styles.topButtonSelected]}><Text style={[styles.topButtonText, this.state.selectBtn == 2 && styles.topButtonTextSelected]}>获取属性</Text></TouchableOpacity>
                        <TouchableOpacity onPress={() => { this.setState({ selectBtn: 3 }) }} style={[styles.topButton, this.state.selectBtn == 3 && styles.topButtonSelected]}><Text style={[styles.topButtonText, this.state.selectBtn == 3 && styles.topButtonTextSelected]}>调用方法</Text></TouchableOpacity>
                        <TouchableOpacity onPress={() => { this.setState({ selectBtn: 4 }) }} style={[styles.topButton, this.state.selectBtn == 4 && styles.topButtonSelected]}><Text style={[styles.topButtonText, this.state.selectBtn == 4 && styles.topButtonTextSelected]}>订阅</Text></TouchableOpacity>
                        <TouchableOpacity onPress={() => { this.setState({ selectBtn: 5 }) }} style={[styles.topButton, this.state.selectBtn == 5 && styles.topButtonSelected]}><Text style={[styles.topButtonText, this.state.selectBtn == 5 && styles.topButtonTextSelected]}>获取Spec</Text></TouchableOpacity>
                    </View>
                    <View >
                    ...
                    <View style={{ height: 60, paddingHorizontal: 10, flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}>
                        <TouchableOpacity onPress={() => { this.state.dataArray.push(''); this.setState({}) }} style={{ width: 70, height: 30, borderColor: '#ccc', borderWidth: 0.5, marginVertical: 10, justifyContent: 'center', alignItems: 'center' }}><Text >添加一条</Text></TouchableOpacity>
                    </View>
                    <TextInput editable={false} style={{height:250,margin:10,borderColor:'#eee',borderWidth:0.5,backgroundColor:'#ccc'}} multiline={true} numberOfLines={0} value={this.state.result}></TextInput>
                </ScrollView>
            </View>
        )
    }
    ...
    _createTextRow(curIndex, title) {
        return (<View key={curIndex} style={{ height: 50, paddingVertical: 8, flexDirection: 'row', }}>
            ...
            <TouchableOpacity onPress={() => {
                ...
                }} style={{ width: 50, height: 30, marginLeft: 5, justifyContent: 'center', alignSelf: 'center' }}><Text>执行</Text></TouchableOpacity>
            <TouchableOpacity onPress={()=>{this._deleteRow(curIndex)}} style={{ width: 50, height: 30, marginLeft: 5, justifyContent: 'center', alignSelf: 'center' }}><Text>删除</Text></TouchableOpacity>
        </View>)
    }
    ...
    _onOpenSubPage(index) {
        ...
        switch (this.state.selectBtn) {
            ...
            case 2:
                ...
                Service.spec.getPropertiesValue([{ did:array[0], siid:parseInt(array[1]), piid: parseInt(array[2])}]).then(res => {
                    this.setState({result:JSON.stringify(res)});
                }).catch(res => {
                    this.setState({result:res.message});
                });
            ...
            case 3:
                ...
                Service.spec.doAction({ did:array[0], siid:parseInt(array[1]), aiid: parseInt(array[2]), inList: array.length>3?this._parseValue(array[3]):null }).then(res => {
                    this.setState({result:JSON.stringify(res)});
                }).catch(res => {
                    this.setState({result:res.message});
                });
...
var styles = StyleSheet.create({
    ...
    topButton: { flex: 1, borderColor: '#ccc', borderWidth: 0.5, backgroundColor: '#eee', alignContent: 'center', justifyContent: 'center' },
