...
import { radiusToken } from 'miot/ui/hyperOSUI';
...
const TestComponent = ({ component: Component, propConfigs, onPropsChange }) => {
              ...
              if (prop.defaultValue !== undefined) {
                ...
                switch (prop.type) {
                  case 'string':
                    ...
                    break;
                  case 'number':
                    objValue[prop.name] = 0;
                    break;
                  case 'boolean':
                    objValue[prop.name] = false;
                    break;
                  case 'enum':
                    objValue[prop.name] = prop.enumOptions?.[0] ?? undefined;
                    break;
                  case 'pass':
                    objValue[prop.name] = undefined;
                    break;
                  case 'array':
                    objValue[prop.name] = [];
                    break;
  ...
  const handleSetNull = (propName) => {
    setPropValues((prev) => ({ ...prev, [propName]: null }));
  };

  const handleSetUndefined = (propName) => {
    setPropValues((prev) => ({ ...prev, [propName]: undefined }));
  };

  const handleBooleanChange = (propName, value) => {
    setPropValues((prev) => ({ ...prev, [propName]: value }));
  };

  const handleEnumSelect = (propName, value) => {
    setPropValues((prev) => ({ ...prev, [propName]: value }));
  };
  ...
  const setDeep = (root, path, val) => {
    ...
    if (keys.length === 1) {
      ...
      if (Array.isArray(root)) {
        ...
        if (!isNaN(idx) && idx >= 0 && idx < root.length) {
          const next = [...root];
          next[idx] = val;
          return next;
        }
  ...
  const handleObjectPropertyUpdate = (propName, propertyName, value) => {
    setPropValues((prev) => ({
      ...prev,
      [propName]: setDeep(prev[propName], propertyName, value),
    }));
  };

  const handleObjectPropertyNull = (propName, propertyName) => {
    handleObjectPropertyUpdate(propName, propertyName, null);
  };

  const handleObjectPropertyUndefined = (propName, propertyName) => {
    handleObjectPropertyUpdate(propName, propertyName, undefined);
  };
  ...
  const handleObjectPropertyPassPreset = (propName, propertyName, presetType) => {
    ...
    switch (presetType) {
      case 'function':
        value = (...args) => {
          console.log(`Function ${ propName }.${ propertyName } called with:`, args);
        };
        break;
      case 'reactNode':
        value = 'ReactNode';
  ...
  const handleObjectPropertyPassCustom = (propName, propertyName, customValue) => {
    try {
      ...
      handleObjectPropertyUpdate(propName, propertyName, parsed);
    } catch {
      handleObjectPropertyUpdate(propName, propertyName, customValue);
    }
  };

  const buildLinkedCallback = (config) => {
    if (config.type !== 'pass' || !config.linkTo) {
      return propValues[config.name];
    }
          ...
          <View style={[styles.propSection, { backgroundColor: COLORS.cardBg, borderBottomColor: COLORS.cardBorder }]}>
            ...
            <View style={styles.optionsRow}>
              <TextInput
                style={[
                  styles.textInput,
                  { flex: 1, backgroundColor: COLORS.inputBg, borderColor: COLORS.inputBorder, color: COLORS.textColor },
                ]}
                value={typeof currentValue === 'string' ? currentValue : ''}
                onChangeText={(text) => {
                  setPropValues((prev) => ({ ...prev, [config.name]: text }));
                }}
                placeholder={currentValue === undefined ? 'undefined' : '输入文本'}
            ...
            {config.passOptions && config.passOptions.length > 0 && (
              <View style={styles.optionsRow}>
                {config.passOptions.map((opt, i) =>
                  renderOptionButton(
                    opt.label,
                    currentValue === opt.value,
                    () => setPropValues((prev) => ({ ...prev, [config.name]: opt.value })),
                    `str-opt-${ i }`
                  )
                )}
              </View>
          ...
          <View style={[styles.propSection, { backgroundColor: COLORS.cardBg, borderBottomColor: COLORS.cardBorder }]}>
            ...
            {isEditing ? (
              <View style={styles.editingContainer}>
                <TextInput
                  style={[styles.textInput, { backgroundColor: COLORS.inputBg, borderColor: COLORS.inputBorder, color: COLORS.textColor }]}
                  value={editingValue}
                  autoFocus
                ...
                <View style={styles.optionsRow}>
                  <TextInput
                    ...
                    onChangeText={(text) => {
                      ...
                      setPropValues((prev) => ({ ...prev, [config.name]: isNaN(num) ? undefined : num }));
                    }}
          ...
          <View style={[styles.propSection, { backgroundColor: COLORS.cardBg, borderBottomColor: COLORS.cardBorder }]}>
            ...
            <View style={styles.optionsRow}>
              {config.enumOptions?.map((option) =>
                renderOptionButton(option, currentValue === option, () => handleEnumSelect(config.name, option), option)
              )}
              {renderOptionButton('undefined', currentValue === undefined, () => handleSetUndefined(config.name))}
            </View>
          </View>
            ...
            {isEditingArray ? (
              <View style={styles.editingContainer}>
                <TextInput
                  ...
                  autoFocus
                  onChangeText={setEditingValue}
                  placeholder='输入数组 (JSON格式): [1, 2, "three"]'
                  placeholderTextColor={COLORS.placeholderColor}
                />
                <View style={styles.optionsRow}>
            ...
            {config.objectProps && config.objectProps.length > 0 && (
              <View style={[styles.objectPropsSection, { borderLeftColor: COLORS.cardBorder }]}>
                {config.objectProps.map((propConfig) => {
                  const propValue = propConfig.name.includes('.')
                    ? propConfig.name.split('.').reduce((obj, k) => obj?.[k], currentValue)
                    : currentValue?.[propConfig.name];
                  return (
                    <View key={propConfig.name} style={styles.objectPropItem}>
                      <Text style={[styles.propLabel, { color: COLORS.titleText }]}>
                        {propConfig.name} ({propConfig.type})
                      </Text>
                      ...
                      {propConfig.type === 'string' && (
                        <View style={styles.optionsRow}>
                          <TextInput
                            ...
                            placeholder="输入文本"
                            placeholderTextColor={COLORS.placeholderColor}
                            onChangeText={(text) => handleObjectPropertyUpdate(config.name, propConfig.name, text)}
                          />
                          {renderOptionButton('undefined', propValue === undefined, () =>
                            handleObjectPropertyUndefined(config.name, propConfig.name)
                          )}
                        </View>
                      )}
                      ...
                      {propConfig.type === 'number' && (
                        <View style={styles.optionsRow}>
                          <TextInput
                            ...
                            onChangeText={(text) => {
                              const num = parseFloat(text);
                              handleObjectPropertyUpdate(config.name, propConfig.name, isNaN(num) ? undefined : num);
                            }}
                          />
                          {renderOptionButton('undefined', propValue === undefined, () =>
                            handleObjectPropertyUndefined(config.name, propConfig.name)
                      ...
                      {propConfig.type === 'boolean' && (
                        <View style={styles.optionsRow}>
                          {renderOptionButton('true', propValue === true, () =>
                            handleObjectPropertyUpdate(config.name, propConfig.name, true)
                          )}
                          {renderOptionButton('false', propValue === false, () =>
                            handleObjectPropertyUpdate(config.name, propConfig.name, false)
                          )}
                          {renderOptionButton('undefined', propValue === undefined, () =>
                            handleObjectPropertyUndefined(config.name, propConfig.name)
                          )}
                      ...
                      {propConfig.type === 'enum' && propConfig.enumOptions && (
                        <View style={styles.optionsRow}>
                          {propConfig.enumOptions.map((option) =>
                            renderOptionButton(
                              option,
                              propValue === option,
                              () => handleObjectPropertyUpdate(config.name, propConfig.name, option),
                              option
                            )
                          )}
                          {renderOptionButton('undefined', propValue === undefined, () =>
                      ...
                      {propConfig.type === 'pass' && (
                        <View>
                          {propConfig.passOptions && propConfig.passOptions.length > 0 && (
                            <View style={styles.optionsRow}>
                              {propConfig.passOptions.map((opt, i) =>
                                renderOptionButton(
                                  opt.label,
                                  propValue === opt.value,
                                  () => handleObjectPropertyUpdate(config.name, propConfig.name, opt.value),
                                  `sub-pass-${ propConfig.name }-${ i }`
                                )
                              )}
                            </View>
                          )}
                          {(!propConfig.passOptions || propConfig.passOptions.length === 0) && (
                            <View style={styles.optionsRow}>
                              <TextInput
                                ...
                                defaultValue=""
                                placeholder="自定义值 (JSON)"
                                placeholderTextColor={COLORS.placeholderColor}
                                onSubmitEditing={(e) => {
                                  const text = e.nativeEvent.text.trim();
                                  if (text) {
                                    handleObjectPropertyPassCustom(config.name, propConfig.name, text);
                                  }
                                }}
                              />
                            </View>
                          ...
                          <View style={styles.optionsRow}>
                            {(!propConfig.passOptions || propConfig.passOptions.length === 0) &&
                              renderOptionButton('f function', typeof propValue === 'function', () =>
                                handleObjectPropertyPassPreset(config.name, propConfig.name, 'function')
                              )}
                            {(!propConfig.passOptions || propConfig.passOptions.length === 0) &&
                              renderOptionButton('ReactNode', propValue === 'ReactNode', () =>
                                handleObjectPropertyPassPreset(config.name, propConfig.name, 'reactNode')
                              )}
                            {renderOptionButton('null', propValue === null, () => handleObjectPropertyNull(config.name, propConfig.name))}
                            {renderOptionButton('undefined', propValue === undefined, () =>
                              handleObjectPropertyUndefined(config.name, propConfig.name)
                            )}
                          </View>
                        ...
                        <View>
                          <View style={styles.optionsRow}>
                            <TextInput
                              ...
                              value={propValue ? JSON.stringify(propValue) : ''}
                              placeholder='输入数组 (JSON格式): [1, 2, "three"]'
                              placeholderTextColor={COLORS.placeholderColor}
                              onChangeText={(text) => {
                                if (text === '') {
                                  handleObjectPropertyUpdate(config.name, propConfig.name, []);
                                }
                              }}
                              onSubmitEditing={(e) => {
                                const text = e.nativeEvent.text.trim();
                                if (text) {
                                  try {
                                    ...
                                    if (Array.isArray(parsed)) {
                                      handleObjectPropertyUpdate(config.name, propConfig.name, parsed);
                                    } else {
                                      alert('输入必须是数组类型');
                                    }
                                  } catch {
                          ...
                          <View style={styles.optionsRow}>
                            {renderOptionButton('[1,2,3]', false, () =>
                              handleObjectPropertyUpdate(config.name, propConfig.name, [1, 2, 3])
                            )}
                            {renderOptionButton('["a","b"]', false, () =>
                              handleObjectPropertyUpdate(config.name, propConfig.name, ['a', 'b'])
                            )}
                            {renderOptionButton('null', propValue === null, () => handleObjectPropertyNull(config.name, propConfig.name))}
                            {renderOptionButton('undefined', propValue === undefined, () =>
                              handleObjectPropertyUndefined(config.name, propConfig.name)
  ...
  return (
    <View style={[styles.container, { backgroundColor: COLORS.pageBg }]}>
      <View style={[styles.previewSection, { backgroundColor: colorToken.surfaceCardTranslucent }]}>
        <View style={[styles.previewContainer]}>
          <Component
            {...propValues}
            {...propConfigs.reduce((acc, config) => {
              if (config.type === 'pass' && config.linkTo) {
                acc[config.name] = buildLinkedCallback(config);
              }
              return acc;
            }, {})}
          />
        ...
        {CATEGORY_ORDER.map((cat) => {
          ...
          return (
            <View key={cat}>
              ...
              {configs.map((config) => (
                <View key={config.name}>{renderPropEditor(config)}</View>
              ))}
            </View>
          );
        })}
        {groupedConfigs.uncategorized.length > 0 &&
          groupedConfigs.uncategorized.map((config) => <View key={config.name}>{renderPropEditor(config)}</View>)}
      </ScrollView>
    </View>
  );
};
...
const styles = StyleSheet.create({
  ...
  previewSection: {
    marginHorizontal: 12,
    borderRadius: radiusToken.cardLarge,
    overflow: 'hidden',
  },
  previewContainer: {
