...
import { Drawer, ButtonGroup, colorToken } from 'miot/ui/hyperOSUI';
...
const ElasticDrawerDemo = () => {
  ...
  useEffect(() => {
    Host.getSafeAreaInsets().then((res) => {
      const navHeight = res?.bottom || 0;
      setSafeBottom(Host.isIphoneXSeries ? navHeight : 0);
      setButtonsBottom(Host.isIphoneXSeries ? navHeight + 28 : 28);
    }).catch(() => {});
  }, []);
  ...
  return (
    <View style={styles.container}>
      <ButtonGroup buttons={outerButtons} />
      ...
      >
        {fullScreen ? (
          <ScrollView
            style={[styles.drawerContent, { marginBottom: showCat ? safeBottom + 28 : buttonsPadding }]} contentContainerStyle={{ paddingBottom: 40 }}
          >
            <TouchableOpacity
        ...
        {!showCat && <ButtonGroup buttons={innerButtons} />}
      </Drawer>
    </View>
  );
};
