...
import { CardContainer, colorToken, Fonts, SliderWithToggle } from 'miot/ui/hyperOSUI';
...
const SliderWithToggleDemo = ({ navigation }) => {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 40 }}>
      ...
      <CardContainer gap={12}>
        <Text style={styles.label}>右侧带开关（开）</Text>
        <View style={styles.sliderWrap}>
          <SliderWithToggle
            sliderProps={{
              value: 60,
            ...
            buttonProps={{
              checked: lightOn,
              colorType: 'blue',
              onChange: (val) => { setLightOn(val); setLightDisabled(!val); },
              icon: <SliderIcon fill={colorToken.contentSecondaryNormal} />,
            }}
          />
        </View>
      </CardContainer>
