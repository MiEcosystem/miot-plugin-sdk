...
import { SmallGridEntrance, colorToken, Fonts, SmallGridContainer, CardContainer, ContainerWithGap } from 'miot/ui/hyperOSUI';
...
const sourceData1 = 
  {
    index: 1,
    title,
    icon: <Circle fill={colorToken.contentPrimaryNormal} />,
  };
...
const SmallGridEntranceDemo = ({ navigation }) => {

  useEffect(() => {
    navigation.setParams({
      right: [{ key: NavigationBar.ICON.MORE, onPress: () => navigation.navigate('SmallGridEntranceConfigDemo', { title: 'SmallGridEntrance 配置调试' }) }],
    });
  }, []);
          ...
          <CardContainer>
            <SmallGridContainer columns={2}>
              {new Array(2).fill(state.sourceData1).map((item, index) => {
                return <SmallGridEntrance key={index} {...item} disabled={state.disabled}/>;
              })}
            </SmallGridContainer>
          </CardContainer>
          <CardContainer>
...
export default SmallGridEntranceDemo;
