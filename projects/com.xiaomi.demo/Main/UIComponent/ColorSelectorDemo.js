...
export default class ColorSelectorDemo extends React.Component {
  ...
  render() {
    let { disable, showIndicator, indicatorRadius, allowIndicatorOverlap, showIndicatorText, indicatorTexts, showBorder, showWhite } = this.state;
    return (<View style={ { flex: 1 } }>
      <ColorSelector
        ref={(ref) => { this.colorSelector = ref; }}
        style={[Styles.colorPicker, { marginTop: 20 }]}
        type={'rgb'}
        colors={['#FF0000', '#FF8000', '#FFFF00', '#2BFF00', '#00FFFF', '#006AFF', '#1500FF', '#FF00FF', '#FF001F']}
        disable={disable}
      ...
      <TouchableOpacity style={ Styles.button } onPress={ () => {
        this.colorSelector.setColorWithIndex('#ff0001', 0);
      } }>
