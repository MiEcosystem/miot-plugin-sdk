...
export declare class IMHRoom {
   ...
   */
  updateName(newName: string): Promise<object>
}
// // @native
// let cachedRooms;
/**
 * @export
 */
export declare class IMiotRoom {
  /**
   * 获取当前家庭所有房间列表
   * @since 10020
   ...
   * 失败时：{"code":xxx, "message":"xxx" }
   */
  loadAllRoom(forceReload?: boolean): Promise<object>;
  /**
   * 使用指定名称创建房间
   * @since 10020
   ...
   * 失败时：{"code":xxx, "message":"xxx" }
   */
  createRoom(name: string): Promise<object>;
  /**
   ...
   * })
   */
  getRoomNames(roomIds: string[]): Promise<object[]>;
}
declare const MiotRoomInstance: IMiotRoom;
