...
export declare class IAccount {
   ...
   */
  load(force?: boolean): Promise<IAccount>;
  /**
   * 获取指定某一账号id的信息
   * @since 10032
   ...
   * 失败时：{"code":xxx, "message":"xxx" }
   */
  getAccountInfoById(accountId: string): Promise<{
    ID: string,
    avatarURL: string,
    birth: string,
    email: string,
   ...
   * @returns {Promise<SimpleAccountInfo[]>}  账号信息列表(数组结构)
   * 成功时：[{ID:xxx, avatarURL: {size_75:xxx,size_90:xxx,...}, icon:xxx, nickName:xxx, userName:xxx}, ...]
   * 失败时：{"code":xxx, "message":"xxx" }
   */
  getAccountInfoList(ids: string[]): Promise<{
    ID: string,
    avatarURL: string,
    icon: string,
    nickName: string,
...
export const CurrentAccount: IAccount