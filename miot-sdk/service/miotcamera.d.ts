...
export interface MISSCommand {
  ...
  MISS_CMD_MOTOR_RESP: 0x113,
  /** < S->C, device motor control response */
  ...
  // 枪球机联动查看
  P2P_PROTOCOL_POINT_WHERE_HIT_WHERE_REQ: 0x130,
  // 切换插件显示模式C-->S, 10109
 ...
 */
export interface MISSConnectState {
 ...
 */
export interface AlarmEventType {
  EventType_All: number,
  EventType_AI: number,
  EventType_Face: number,
  EventType_KnownFace: number,
  EventType_PeopleMotion: number,
  EventType_ObjectMotion: number,
  EventType_BabyCry: number
};
/**
 * @export
 */
export declare class IMiotCamera {
  /**
   * 连接设备
   * @param {string} callbackName 链接状态变更回调 { state: MISSConnectState, error: MISSError }
   * @param did
   */
  connectToDeviceWithStateChangeCallBack(callbackName: string, did?: string): void;
  /**
   * 断开连接设备
   * @since 10033
   */
  disconnectToDevice(did?: string): void;
  /**
   * 发送miss命令到设备
   ...
   */
  sendP2PCommandToDevice(command: MISSCommand, paramsJson: object, did?: string): Promise<number>;
  /**
   ...
   * @returns {Promise<number>} a promise with return code
   */
  sendP2PCommandToDeviceWithStringParam(command: MISSCommand, paramStr: string, did?: string): Promise<number>;
  /**
   ...
   * @returns {Promise<number>} a promise with return code
   */
  sendP2PCommandToDeviceWithBase64Param(command: MISSCommand, base64Param: string, did?: string): Promise<number>;
  /**
   * 注册接收命令回调
   * @param {string} callbackName 收到p2p command回调 { command: MISSCommand, data: Object/Base64String }
   */
  bindP2PCommandReceiveCallback(callbackName: string, did?: string): void;
  /**
   * 发送RDT命令到设备
   * @param {object} params json data
   * @returns {Promise<number>} a promise with return code
   */
  sendRDTJSONCommandToDevice(params: object, did?: string): Promise<number>;
  /**
   * 发送RDT命令到设备
   * @param {string} params base64 encoded data
   * @returns {Promise<number>} a promise with return code
   */
  sendRDTCommandToDevice(params: string, did?: string): Promise<number>;
  /**
   * 注册接收RDT命令回调
   * @param {string} callbackName 收到RDT回调 { data: Object/Base64String }
   */
  bindRDTDataReceiveCallback(callbackName: string, did?: string): void;
  /**
   * 打开报警视频页面
   ...
   * @updated 10047
   */
  showAlarmVideos(localRecognizeEvents: AlarmEventType | number, did = Device.deviceID, isNewPlugin?: boolean): void;
  /**
   * 打开回看页面
   * @since 10048
   ...
   */
  showPlaybackVideos(data: string, did?: string): void;
  /**
   * 打开长时间无人出现页面
   * @since 10054
   * @param {data} jsonobj
   * @example
   */
  openLongTimeNobody(data: string, did?: string): void;
  /**
   * 打开每日故事设置
   * @since 10054
   * @param {data} jsonobj
   * @example
   */
  openDailyStorySetting(data: string, did?: string): void;
  /**
   * 打开宝宝睡眠设置
   * @since 10054
   * @param {data} jsonobj
   * @example
   */
  openBabySleepSetting(data: string, did?: string): void;
  /**
   * 打开IDM设置
   * @since 10054
   * @param {data} jsonobj
   * @example
   */
  openIDMSetting(data: string, did?: string): void;
  /**
   * 打开相册
   * @since 10051
   ...
   *         Service.miotcamera.showPlaybackVideos(JSON.stringify(data));
   */
  showAlbum(data: string, did?: string): void;
  /**
   * 打开设备相册中最新的图片或视频
   * @since 10051
   ...
   */
  showLastAlbumMediaFile(data: string, did?: string): void;
  /**
   * 获取设备对应的相册名字
   * @since 10051
   */
  getAlbumName(did?: string): Promise<string>;
  /**
   * 打开云储存页面
   * @since 10033
   ...
   */
  showCloudStorage(supportHevc: boolean, useV2API: boolean, did?: string, cloudStoragePurchaseUrl?: string, directGoPurchasePage?: boolean, params?: object): void;
  /**
   * 打开云储存设置页面
   * @since 10033
   * @param {did} 默认参数 did
   ...
   */
  showCloudStorageSetting(did?: string, aSettingUrl?: string): void;
  /**
   * 打开报警视频播放页面
   * @since 10037
   * @param data  jsonobj=>str 从push点击跳转进来后，如果是smartscene 就把extra字段捞出来放到这里
   */
  openAlarmVideoPlayer(data: ojbect): void;
  /**
   * @param model
   * @param did
   ...
   * @since 10047
   */
  loadMonitoringDetail(model?: string, did?: string): Promise<string>
  /**
   * 打开人脸识别页面
   * @since 10054
   * @param {BOOL} isVip
   ...
   */
  showFaceRecognize(isVip: boolean, did?: string, aFreeFaceSt: string | null, aBuyVipUrl = string | null): void
  /**
   *
   ...
   * @since 10036
   */
  bindBPSReceiveCallback(callbackName: string, did?: string): void
  /*
   * 拉取当前正在播放时间戳 js端控制拉取节奏
   */
  getCurrentFrameInfo(did?: string): Promise<string>
  /**
   * 执行FFmpeg命令
   ...
   */
  ffmpegCommand(command: string, callbackName: string, complete: (isSuccess: boolean, errInfo: object) => void): void
  /**
   * 下载m3u8视频并合成mp4
   * @since 10038
   * @param fileId
   ...
   */
  downloadM3U8ToMP4(fileId: string, filePath: string, callbackName: string, isAlarm?: boolean, videoCodec?: string, did?: string, extra: object): void;
  /**
   ...
   */
  @report
  cancelDownloadM3U8ToMP4(fileId: string, callbackName: string, isAlarm?: boolean, did?: string): void;
  /**
   ...
   *    progress : 下载进度0 - 100 (state = onProgress时才有)
   */
  downloadM3U8ToMP4V2(fileId: string, filePath: string, callbackName: string, isAlarm?: boolean, videoCodec?: string, did?: string, transcode?: boolean): void;
  /**
   ...
   * @param {*} did
   */
  cancelDownloadM3U8ToMP4V2(fileId: string, callbackName: string, isAlarm?: boolean, did?: string): void;
  /**
   * 获取报警视频m3u8播放地址
   * @since 10037
   ...
   * @param videoCodec 视频编码如 "H264", "H265"
   */
  getVideoFileUrl(fileId: string, isAlarm: boolean, videoCodec: string, did?: string): Promise<string>;
  /**
   ...
   */
  getFileIdImage(imgStoreId: string, did?: string): Promise<string>;
  /**
   * 获取人脸图片接口
   * @since 10040
   ...
   * @returns {Promise<String>} 文件路径
   */
  getFaceImgWithDid(faceId: string, did?: string): Promise<string>;
  /**
   * 获取通用图片接口（如云存储视频缩略图）
   * @since 10041
   ...
   * @returns {Promise<String>} 文件路径
   */
  getCommonImgWithParams(imgId: string, hostParams: string, pathParams: string, did?: string): Promise<string>;
  /**
   ...
   * @returns {null}
   */
  setTimelinePlaybackMode(isTimelinePlayback: boolean, did?: string): void
  /**
   ...
   * @returns {null}
   */
  bindTimelinePlaybackEndListener(timelinePlaybackEndListenerName: string, did?: string): void;
  /**
   * 获取当前语音对讲过程中的音量大小
   * @since 10038
   * return {promise}
   */
  getCurrentSpeakerVolumn(did?: string): Promise<object>;
  /**
   * 设置当前model是不是miss固件
   * @param boolean isMissFirmware  是否是miss固件，true 是； false tutk固件。
   * @since 10038
   */
  setCurrentDeviceIsMissFirmware(isMissFirmware: boolean, did?: string): void;
  /**
   * 打开摄像机NAS存储设置页面。
   *
   * @since 10038
   *
   */
  showNASSetting(did?: string): void;
  /**
   * 开启悬浮窗模式
   * @since 10039
   */
  openFloatWindow(did?: string): Promise<object>;
  /**
   * 关闭悬浮窗
   * @since 10040
   */
  closeFloatWindow(did?: string): Promise<void>;
  /**
   * 绑定摄像头的报警视频到微信米家公众号里，有报警视频推送到微信公共号
   * @since 10040
   */
  tryBindAlarmNotifyWithWechatMijia(did?: string): Promise<object>
  /**
   ...
   * @since 10041
   */
  queryDevicePassword(did?: string): Promise<string>;
  /**
   ...
   * @since 10047
   */
  setCurrentDeviceUseFixedUid(isUseFixedUid: boolean, did?: string): Promise<void>;
  /**
   * 使用chacha20_xor解密大文件
   ...
   */
  decryptBigFile(fileData: string, nonce: string, did?: string): Promise<string>
  /**
   * 使用chacha20_xor解密小文件
   ...
   * @since 10041, for ios from 10062
   */
  decryptSmallFile(fileData: string, nonce: string, did?: string): Promise<string>
  /**
   ...
   * @since 10042
   */
  markCurrentDeviceUseHualaiEncrypted(isEncrypted: boolean, did?: string): void
  /**
   ...
   * @since 10042
   */
  setCurrentDeviceUseFixedRdtChannel(useFixedRdtChannel: boolean, did?: string): void
  /**
   * 设置开启/关闭ijk解码。
   ...
   * @since 10043
   */
  setUseIjkDecoderGlobal(isEnableIjk: boolean, did?: string): void
  /**
   ...
   */
  setCurrrentVoiceChangerType(simpleRate: number, type: number, channel?: number, did?: string, extra: object): void;
  /**
   * 打开门铃的带屏设备联动页面
   ...
   * @since 10044
   */
  showScreenLinkagePage(isMultiChoice: boolean, screenCount: number, did?: string, events?: object[]): void;
  /**
   * 打开云存下载列表页面
   * @param {string} did
   *
   * @since 10046
   */
  openCloudSettingDownloadListPage(did?: string): void
  /**
   * 下载云存视频到 云存管理-》下载列表里
   ...
   * @since 10047
   */
  downloadCloudVideoIntoCloudSetting(did: string, fileId: string, isAlarmFile: boolean, startTime: number, duration: number, register?: boolean, thumbId?: string): Promise<object>;
  /**
   * 下载mp4类型的云存视频
   ...
   * @since 10047
   */
  downloadCloudVideoMp4(fileId: string, stoId: string, did?: string): Promise<string>;
  /**
   ...
   * @since 10047
   */
  convertG711VideoIntoAACVideo(filePath: string, audioParam: object): Promise<void>;
  /**
   ...
   *  @since 10049
   */
  reactNativeVideoScreenShot(viewRef: number, imagePath: string): Promise<string>;
  /**
   ...
   * @since 10052
   */
  launchRecommend(srid: string, did?: string, model?: string): Promise<object>;
  /**
   * 打开推荐运营位页面
   ...
   */
  showOperationBannerPage(data: object, did?: string): void;
  /**
   * @param isPadFullScreen 是否希望进入pad下的全屏， 设置为true，会把画面铺满屏幕，设置为false，会恢复画面
   */
  @report
  enterFullscreenForPad(isPadFullScreen: boolean): void;
  /**
   * 过滤视频帧
   ...
   */
  setFrameFilter(aFilter: object, aCbName: string, aDid?: string): void
  /**
   * 发送tutk 函数类型命令
   ...
   */
  callTutkSpecial(args: { name: string }, aDid?: string): Promise<object>;
  /**
   * 发送audio 数据
   ...
   */
  sendAudioData(aDatInBase64: string, aHeaderInBase64: string, aCodeId?: MISSCodec, aDid?: string): Promise<object>;
  /**
   ...
   * @since 10056
   */
  bindMoveTypeChangeCallback(callbackName: string, did?: string): Promise<void>;
  /**
   * @since 10058
   * @param {拍照或者相册得到的图片路径} imagePath
   * @param {did} did
   */
  uploadImageToCameraServer(imagePath: string, did?: string): Promise<object>;
  /**
   * @since 10063
   ...
   */
  audioCodec(params: object): Promise<object>;
  /**
   * @since 10065
   * params{filePath, transformation(AES/ECB/PKCS5Padding), password}
   */
  decryptFile(params: object): Promise<object>;
  /**
   * @since 10065
   * params{filePath, transformation(AES/ECB/PKCS5Padding), password}
   */
  encryptFile(params: object): Promise<string>
  /**
   ...
   * @returns {Promise<String>} 文件路径
   */
  getVideoFileUrlV2(hostPrefix: string, path: string, param: object): Promise<string>;
  /**
   * 下载m3u8视频并合成mp4，支持合成mp4时统一分辨率，避免视频花屏
   * @since 10066
   ...
   *    progress : 下载进度0 - 100 (state = onProgress时才有)
   */
  downloadM3U8ToMP4ByUrl(fileId: string, url: string, filePath: string, callbackName: string, did?: string): void;
  /**
   *
   * @param {Device.deviceID} did
   * @returns result是tutkSessionRead base64编码后的结构体
   */
  tutkSessionRead(did?: string): Promise<object>;
  /**
   ...
   */
  tutkSessionWrite(ioCtrlBuffInBase64: string, did ?: string): Promise<object>;
  /**
   * 门锁远程开锁
   * @since 10102
   * @param {Device.deviceID} did
   */
  startUnlockDevice(did ?: string): Promise<void>;
}
declare const MiotCameraInstance: IMiotCamera;
