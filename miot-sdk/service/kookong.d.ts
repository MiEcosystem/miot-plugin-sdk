...
export declare class IKookong {
   ...
   */
  registerWithKey(apikey: string, checkid: string): Promise<{code: number, message: string}>;
  /**
   *  创建 带状态控制的空凋控制实例
   *
   *  @since 10035
   ...
   */
  createZipACManager(managerIdentifier: string, remoteid: string, irData: object, array?: any[]): Promise<{code: number, message: string}>;
  /**
   *  创建 带状态控制的空凋控制实例
   *
   *  @since 10035
   ...
   *  {"code":-1,"message":"identifier cannot be null"}
   */
  createNonACManager(managerIdentifier: string, irData: object): Promise<{code: number, message: string}>;
  /**
   ...
   *  {"code":-1,"message":"identifier cannot be null"}
   */
  removeACManager(managerIdentifier: string): Promise<{code: number, message: string}>;
  /**
   ...
   */
  @report
  canControlWithType(managerIdentifier: string, type?: number): Promise<{code: number, message: string}>;
  /**
   *  获取当前控制的指定type的值
   *
   *  @since 10035
   ...
   *  {"code":-102,"message":"not found manager, make sure you have already called createZipACManager yet"}
   */
  getCurrentValueWithType(managerIdentifier: string, type: number): Promise<{code: number, message: string}>;
  /**
   ...
   *  {"code":-102,"message":"not found manager, make sure you have already called createZipACManager yet"}
   */
  getAllSupportValueWithType(managerIdentifier: string, type: number): Promise<{code: number, message: string}>;
  /**
   *  发送控制指令
   *
   *  @since 10035
   ...
   *  {"code":-102,"message":"not found manager, make sure you have already called createZipACManager yet"}
   */
  changeStateValueForType(managerIdentifier: string, stateValue: number, type: number): Promise<{code: number, message: string}>;
}
declare const Kookoong: IKookong;
