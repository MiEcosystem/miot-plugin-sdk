    ...
    Service.smarthome.getFirmwareUpdateInfo(Device.deviceID)
      .then((res) => {
        ...
        if (Device.isOwner) {
          ...
          if (res.needUpgrade === true) {
            if (this.state.showDot.indexOf(AllOptions.FIRMWARE_UPGRADE) === -1) {
              this.setState({ showDot: [...this.state.showDot, AllOptions.FIRMWARE_UPGRADE] });
            }
          } else {
            if (this.state.showDot.indexOf(AllOptions.FIRMWARE_UPGRADE) !== -1) {
