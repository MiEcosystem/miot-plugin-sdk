...
export default class CommonSetting extends React.Component {
  ...
  static commonSettingReportLog(...logs) {
  ...
  render() {
    ...
    if (['6', '17'].indexOf(Device.type) === -1 && ['light'].indexOf(modelType) !== -1) {
      ...
      CommonSetting.commonSettingReportLog('[Debug] CommonSetting in', ' type: ', Device.type, 'modelType ', modelType);
    }
    CommonSetting.commonSettingReportLog('[Debug] CommonSetting outside', ' type: ', Device.type, 'modelType ', modelType);
    // 管理组设备
    ...
    const items = keys.map((key) => {
      ...
      if (item) {
        ...
        Service.smarthome.reportLog('*****check Device.needUpgrade', Device.needUpgrade);
        Service.smarthome.reportLog('*****check needShowUpgradeRedDot', this.state.needShowUpgradeRedDot);
        Service.smarthome.reportLog('*****check firmwareUpgradeDotClicked', firmwareUpgradeDotClicked);
        ...
        if (key === AllOptions.FIRMWARE_UPGRADE && !item.showDot) {
          ...
          Service.smarthome.reportLog('*****innercheck Device.needUpgrade', Device.needUpgrade);
          Service.smarthome.reportLog('*****innercheck needShowUpgradeRedDot', this.state.needShowUpgradeRedDot);
