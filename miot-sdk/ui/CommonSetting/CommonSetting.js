...
function fetchPowerMode() {
  return new Promise((resolve, reject) => {
    Device.getDeviceWifi().callMethod('miIO.get_powermode', []).then((res) => {
      console.log(`fetchPowerMode() miIO.get_powermode res=${ JSON.stringify(res) }`);
      if (null != res.result && null != res.result.powermode) {
        let pm = res.result.powermode;
        switch (pm) {
          case 0:
          case 1:
            resolve(pm);
...
// 功率模式
const selectIndexArray = [
  {
    title: strings.ghostPowerMode
  }, {
    title: strings.standardPowerMode
  }
];
...
export default class CommonSetting extends React.Component {
  ...
  getCommonSetting(state) {
    let { modelType, productBaikeUrl, roomInfo, freqFlag, freqCameraFlag, freqCameraNeedShowRedPoint, pluginCategory, powerCategory, multipleKeyisOn, keyNum, cloudStorageOn } = state || {};
    ...
    let ret = {
      ...
      [AllOptions.POWER_MODE]: {
        title: strings.powerMode,
        value: selectIndexArray[powerCategory].title,
        onPress: () => {
          this.setState({
            dialogShow: true
          });
  ...
  constructor(props, context) {
    ...
    this.state = {
      ...
      hasStdPlugin: false,
      dialogVisible: false,
      supportPowerMode: false, // APP上预留功率模式配置通道，可通过RPC控制模组进行功率模式切换。
      dialogShow: false,
  ...
  componentDidMount() {
    ...
    if (Device.type === Device.DEVICE_TYPE.DUAL_MODEL_DEVICE || Device.type === Device.DEVICE_TYPE.WIFI_SINGLE_MODEL_DEVICE) {
      ...
      fetchPowerMode().then((powermode) => {
        this.commonSetting = this.getCommonSetting({
          ...this.state,
          powerCategory: powermode
        });
        this.setState({ 
          supportPowerMode: true,
          powerCategory: powermode
        });
      }).catch(() => {});
    }
  ...
  render() {
    let { modelType, productBaikeUrl, freqCameraNeedShowRedPoint, showMultipleKey, hasStdPlugin, supportPowerMode, powerCategory, pluginCategory, showMemberSetKey, isSingleSwitch, showDeviceService } = this.state;
    let requireKeys1 = [
    ...
    if (supportPowerMode) {
      requireKeys1.push(AllOptions.POWER_MODE);
    }
    if (hasStdPlugin) {
      // requireKeys1.push(AllOptions.POWER_MODE);
      ...
      <View style={styles.container}>
        ...
        {supportPowerMode ?
          <ChoiceDialog
            ...
            options={selectIndexArray}
            selectedIndexArray={[powerCategory]}
            onDismiss={() => {
