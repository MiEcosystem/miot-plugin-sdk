...
//全局事件处理
export default (globalEventProfiles, MIOTEventEmitter) => {
        ...
        Object.keys(eventsDef).forEach(name => {
            ...
            if (def.forever) {
                ...
                createProfile(emitter => {
                    const ret = {subscription:MIOTEventEmitter.addListener(realname,
                        def.forever(emitter)
                    )};
                    return () => {
                        if (ret.subscription) {
            ...
            if (def.always) {
                ...
                createProfile(emitter => {
                    ...
                    emitter.onAddListener = (callback) => {
                        if (!ret.subscription) {
                            ret.subscription = MIOTEventEmitter.addListener(name, def.always(emitter));
                        }
                        return callback;
                    }
                    emitter.onRemoveListener = (callback) => {
                        if (emitter.isEmpty()) {
                            close();
                        }
                    }
