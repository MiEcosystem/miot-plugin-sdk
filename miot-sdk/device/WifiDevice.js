...
export default class IDeviceWifi {
    ...
    */
    ...
    callMethod(method, args, extraPayload = {}) {
        ...
        //@mark andr done
        return new Promise((resolve, reject) => {
            console.log("device-->callMethod", BasicDevice.deviceID, this);
            native.MIOTDevice.callMethod(BasicDevice.deviceID, method,
    ...
    callMethodFromCloud(method, args, extraPayload = {}) {
        ...
        return new Promise((resolve, reject) => {
            if (native.isAndroid) {
                native.MIOTDevice.callMethodFromCloud(BasicDevice.deviceID, method,
                ...
                //need deviceID
                //args._miot_device_id = BasicDevice.deviceID;
    ...
    callMethodFromLocal(method, args, extraPayload = {}) {
        ...
        return new Promise((resolve, reject) => {
            if (native.isAndroid) {
                native.MIOTDevice.callMethodFromLocal(BasicDevice.deviceID, method,
                    ((typeof (args) === "string") ? args : JSON.stringify(args)),
                    ((typeof (extraPayload) === "string") ? extraPayload : JSON.stringify(extraPayload)),
                    (ok, res) => {
                        if (ok) {
    ...
    localPing() {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (native.isAndroid) {
                native.MIOTDevice.localPingWithCallback(BasicDevice.deviceID, callback)
            } else {
    ...
    subscribeMessages(...propertyOrEventNames) {
        ...
        return (new Promise((resolve, reject) => {
            native.MIOTDevice.subscribeMessages(BasicDevice.deviceID, propertyOrEventNames, (ok, subscribeId) => {
                if (ok) {
                    ...
                    const stop = cache => {
                        if (cache.deviceMessages) {
                            ...
                            if (def) {
                                ...
                                unsub && unsub(BasicDevice.deviceID, propertyOrEventNames, def.lastId, _ => { });
                    ...
                    if (unsub) {//区分 android ios，ios 没有实现unsubscribeMessages内部重复订阅
                        PackageExitAction.register(stop, cache => {
                            ...
                            def.timer = NativeTimer.addListener(() => {
                                ...
                                native.MIOTDevice.subscribeMessages(BasicDevice.deviceID, propertyOrEventNames, (ok, newId) => {
                                    if (ok && newId) {
    ...
    checkFirmwareUpdateAndAlert() {
        //@native :=> promise {}
        if (BasicDevice.isShared || BasicDevice.isVirtualDevice || !BasicDevice.isOnline || !BasicDevice.deviceID) {
        ...
        // if (BasicDevice.type !== 0 && BasicDevice.type !== 8) { // 如果不是Wi-Fi设备也不是双模设备
        ...
        return new Promise((resolve, reject) => {
            ...
            let check_reqs = [{ did: BasicDevice.deviceID }];
    ...
    */
    checkIsHomeKitDevice() {
        ...
        return new Promise((resolve, reject) => {
            native.MIOTHost.isHomeKitDevice(BasicDevice.model, (ok, res) => {
                if (ok) {
                    resolve(res);
                } else {
                    reject(res);
    ...
    checkHomeKitConnected() {
        ...
        return new Promise((resolve, reject) => {
            native.MIOTHost.isHomeKitConnected(BasicDevice.deviceID, (ok, res) => {
                if (ok) {
                    resolve(res);
                } else {
                    reject(res);
    ...
    bindToHomeKit() {
        ...
        return new Promise((resolve, reject) => {
            native.MIOTHost.addToHomeKit(BasicDevice.deviceID, BasicDevice.model, (ok, res) => {
                if (ok) {
                    resolve(res);
                } else {
                    reject(res);
