...
export class BasicDevice {
  ...
  */
  ...
  /**
   * 获取设备实际所在城市ID（基于设备IP，由云端判定）
   * 用于合规判断，city_id 410 表示设备在韩国
   * 注意：与 getHomeLocation 返回的 cityId（家庭配置位置）不同
   * @since 10118
   * @returns {Promise<Object>} 成功: {city_id: "101190101", did: "xxx"}
   * reject: {code: xxx, message: xxx}
   * @example
 ...
 */
  ...
  getAllDeviceList() {
    ...
    let isSupportModel = supportModelArray.includes(this.model);
    if (isSupportModel || specPluginNames.includes(native.MIOTPackage.packageName)) {
      return new Promise((resolve, reject) => {
