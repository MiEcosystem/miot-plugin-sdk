export default {
  ...
  updateWarningTip: 'Не выключайте и не используйте устройство во время обновления',
  basicPerformanceUpdate: 'Базовая производительность',
  productFunctionUpdate: 'Характеристики продукта',
  updateFailed: 'Ошибка обновления',
  partialUpdateFailed: 'Не удалось завершить часть этого обновления',
  checkUpdateFailed: 'Не удалось проверить наличие обновлений',
  updated: 'Обновлено',
  deviceOfflineCheckNetwork: 'Устройство находится в автономном режиме. Проверьте параметры сети.',
  checkNetworkError: 'Ошибка сети. Подождите немного и повторите попытку.',
  alreadyLatest: 'Обновление не требуется',
  ...
  cloutStorageSetting: 'Дополнительные настройки облачного хранилища',
  open: 'Вкл.',
  close: 'Откл.',
  other: 'Другое',
  ...
  // 房间
  room: 'Комната',
  room_nameInputTips: 'Введите название комнаты',
  ...
  fireSafeCodeTitle: 'Кодекс пожарной безопасности устройств',
  ...
  switch_dia_msg_repeatScene: 'Эта кнопка уже имеет автоматизацию одним нажатием. Подключение устройства может привести к его повторному запуску. Удалить подключенное устройство?',
  common_loading: 'Секунду…',
  ...
  usedOnMiHome: 'Использовать с Xiaomi Home',
  operation_failed: 'Не удалось выполнить это действие',
  operation_cancel: 'Действие отменено',
  operation_success: 'Готово',
  select_control_type: 'Выберите тип элемента управления',
  non_smart_device_wired: 'Управление не смарт-устройствами через проводное соединение',
  control_smart_lights_group: 'Управление смарт-светильниками или группами смарт-светильников в этом доме',
  switch_double_control: 'Управление двумя выключателями',
  link_other_switches_intercontrol: 'Подключитесь к другим умным выключателям, чтобы они управляли друг другом',
  execute_scene: 'Запустить сценарий',
  execute_manual_scenes_home: 'Запустите существующий ручной сценарий (например, «Возвращение домой»)',
  more_smart_devices: 'Больше смарт-устройств',
  control_other_devices_home: 'Управляйте другими смарт-устройствами в этом доме (например, кондиционером)',
  power_switch: 'Выключатели тока',
  turn_off_light_offline: 'Если вы выключите его, подключенный умный светильник потеряет питание и перейдет в автономный режим',
  manage_switch_power: 'Включайте и выключайте выключатель. Чтобы сохранить доступ к смарт-светильникам через сеть, оставьте его включенным.',
  automation_toggle_btn: 'Переключиться',
  save: 'Сохранить',
  add_device_type: 'Добавить тип устройства',
  common_brand_produced_by: 'Разработано ${}',
  automation_add_control_device: 'Добавить устройства',
  automation_edit_btn: 'Изменить',
  switch_autoCtrl_confirmToSmart_desc: 'После переключения кнопка больше не будет включать или выключать выключательи и может использоваться для управления смарт-устройствами в этом доме.',
  switch_autoCtrl_confirmToSmart_title: 'Переключиться в беспроводной режим?',
  switch_autoCtrl_confirmToSpeedMode_desc: 'В суперскоростном режиме автоматизации двойного нажатия и нажатия и удержания не будут работать.',
  switch_autoCtrl_confirmToSpeedMode_title: 'Переключиться в суперскоростной режим?',
  switch_autoCtrl_confirmToTraditional_desc: 'После переключения кнопка больше не будет управлять смарт-устройствами и связанные с ней автоматизации не будут выполняться.',
  switch_autoCtrl_confirmToTraditional_title: 'Переключиться в проводной режим?',
  switch_autoCtrl_ctrlSection: 'Управление',
  switch_autoCtrl_modeSection: 'Режим',
  switch_autoCtrl_powerSwitch_confirmOff_desc: 'При отключении подключенные устройства отключаются от сети и выключаются',
  switch_autoCtrl_powerSwitch_title: 'Управление питанием',
  switch_autoCtrl_selectControlDevice_title: 'Выбрать устройства',
  switch_autoCtrl_smartMode_desc: 'Используйте кнопку для управления смарт-устройствами, такими как смарт-светильники или смарт-выключатели',
  switch_autoCtrl_smartMode_title: 'Беспроводной режим',
  switch_autoCtrl_traditionalMode_desc: 'Используйте этот режим при подключении к стандартным устройствам, таким как обычные светильники или вытяжные вентиляторы',
  switch_autoCtrl_traditionalMode_title: 'Проводной режим',
  switch_autoCtrl_wirelessHint_title: 'После перехода в режим беспроводной связи доступны автоматизации с однократным нажатием, двойным нажатием, а также нажатием и удержанием',
  switch_controlType_advancedConfig: 'Дополнительная настройка',
  switch_controlType_manualScene_desc: 'Запустить существующие ручные сценарии (например, «Возвращение домой»)',
  switch_controlType_manualScene_title: 'Ручное управление',
  switch_controlType_otherSmartDevice_desc: 'Управляйте другими смарт-устройствами в этом доме',
  switch_controlType_otherSmartDevice_title: 'Создать автоматизацию',
  switch_controlType_quickConfig: 'Быстрая настройка',
  switch_controlType_smartLight_desc: 'Включает и выключает смарт-светильники',
  switch_controlType_smartLight_title: 'Смарт-светильники',
  switch_controlType_switchDualControl_desc: 'Включение и выключение других смарт-выключателей',
  switch_controlType_switchDualControl_title: 'Управление двумя выключателями',
  switch_layoutDetail_editIcon: 'Изменить значок',
  switch_layoutDetail_name: 'Имя',
  switch_layoutDetail_room: 'Комната',
  switch_list_empty_light: 'В этом доме нет поддерживаемых светильников',
  switch_list_empty_manualScene: 'В этом доме нет ручного управления. Добавьте со страницы Xiaomi Home.',
  switch_list_empty_switch: 'В этом доме нет поддерживаемых выключателей',
  switch_menu_deleteConfig: 'Удалить конфигурацию',
  switch_multikey_iconCategory_default: 'По умолчанию',
  switch_multikey_iconCategory_light: 'Светильники',
  switch_multikey_iconCategory_other: 'Другое',
  switch_multikey_iconSelect_title: 'Значок',
  switch_multikey_roomSelect_createFail: 'Не удалось создать комнату',
  switch_multikey_roomSelect_createNew: 'Создать комнату',
  switch_multikey_roomSelect_inputPlaceholder: 'Введите имя комнаты',
  switch_nav_subtitle_buttonControlLight_v2: 'Одно нажатие включает или выключает выбранный светильник',
  switch_nav_subtitle_buttonControlManualScene_v2: 'Одиночное нажатие запускает все выбранные ручные элементы управления',
  switch_nav_subtitle_buttonControlSwitch_v2: 'Одно нажатие включает или выключает выбранный выключатель',
