...
class IFile {
  ...
  */
   ...
   */
  @report
  generateImageWithWatermarkParams(dirName, saveFilePath, watermarkParams) {
     return Promise.resolve(false)
  }
  /**
   * 保存指定图片文件到以did命名的相册中
   ...
   */
  @report
  saveImageToPhotosDidAlbum(fileName, customDirName = null, deviceID = undefined, watermarkParams = null) {
     return Promise.resolve(false)
  }
  /**
   ...
   */
    ...
    mergeVideosUseHarewareAccelerateV2(firstPath, secondPath, params = {}, deviceID = undefined, callbackEvent) {
      if (!isIOS) {
        return Promise.reject({ "code": -1, "message": "mergeVideosUseHarewareAccelerateV2 is only support ios" });
      }
       return Promise.resolve(false)
    }
    /**
