...
class IUi {
  ...
  * @example
   ...
   * @since 10115
   * @example
   * Host.ui.openRemoteControlDialog()
   */
  @report
  openRemoteControlDialog(did) {
  }
  /**
   * 打开本地直连控制关联页面
  ...
  /**
   * 跳转自动化配置新增页面，并预填充触发器配置
   * @since 10116
   * @param {object} params 自动化配置参数
   * @param {Array} params.trigger 触发器列表
   * @param {string} params.trigger[].did 设备 did
   * @param {string} params.trigger[].sc_id 触发条件 sc_id
   * @param {boolean} params.trigger[].value 触发值
   * @param {boolean} params.trigger[].ignore_plugin_id 是否忽略插件 id，传 true 则不跳转到插件详情页
   * @
   * @example
   * Host.ui.fillAutomationConfig({
   *   trigger: [
   *     { did: 'xx', sc_id: 'xx', value: true }
   *   ]
   * });
   */
  @report
  fillAutomationConfig(params) {
  }
