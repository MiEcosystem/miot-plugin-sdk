...
export default {
  ...
  /**
   * OneTrack 打点
   * @param {String} event 打点事件
   * @param {String} tip 打点tip
   * @param {JSON} props 打点参数
   */
  oneTrack(event, tip, props) {
    return new Promise((resolve, reject) => {
      native.MiWearHost.oneTrack(event, tip, props, (isSuccess, result) => {
        if (isSuccess) {
          resolve(result);
        } else {
          reject(result);
  ...
  /**
   * 跳转原生路由
   * @param {String} instance 页面标识符
   * @param {JSON} params 参数
   * @returns 
   */
  openNativeUrl(instance, params) {
    return new Promise((resolve, reject) => {
      native.MiWearHost.openNativeUrl(instance, params, (isSuccess, result) => {
        if (isSuccess) {
          resolve(result);
        } else {
          reject(result);
