...
class DragGear extends React.Component {
  static propTypes = {
  ...
  constructor(props, context) {
    ...
    this.margin = margin;
    this.containerWidth = containerWidth; // console.log(`选项宽度 ${ optionWidth } 间距 ${ margin } 总体宽度 ${ containerWidth }`);
    // 也不能太拥挤吧

    if (this.optionWidth < 20) {
      this.showNothing = true;
      Logger.warn('在目前maxWidth下显示不了这么多选项，请重新规划');
      return;
    }
  ...
  _onPanResponderGrant(e) {
    // 放大动画
    Animated.timing(this.state.scale, dragStartAnimationConfig).start(); // 为了准确确定释放位置，需要在起手的时候，计算出手势触摸点和中心点的偏差

    const {
      pageX
    } = e.nativeEvent;
    this.offset = pageX - this.currentCoord;
    this.pressToChoose = false; // console.log('⬇️⬇️⬇️⬇️⬇️⬇️⬇️拖拽开始⬇️⬇️⬇️⬇️⬇️⬇️⬇️');
  ...
  renderOptions() {
    const style = StyleSheet.flatten([this.props.normalStyle, {
    ...
    return this.props.options.map((option, index) => {
      return <Clickable key={option} allowFontScaling={this.props.allowFontScaling} numberOfLines={this.props.numberOfLines} onPress={() => this.onPress(index)} text={option} select={selectIndex === index} style={style} textStyle={this.props.textStyle} {...getAccessibilityConfig({
    ...
    return <Block panHandlers={this.panResponder.panHandlers} style={[position, panStyle]}>
        <View style={touchArea}>
          <View style={innerCircle}>
            ...
            accessible: false
          })} allowFontScaling={this.props.allowFontScaling} numberOfLines={this.props.numberOfLines}>
  ...
  render() {
    ...
    return <View onLayout={() => this.calculateCoord()} ref={container => this._container = container} style={[this.props.containerStyle, styles.container, container]}>
        {this.renderOptions()}
  ...
  onPress(index) {
    ...
    Animated.sequence([Animated.timing(this.state.scale, dragStartAnimationConfig), Animated.timing(this.state.pan, {
